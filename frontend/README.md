...
这是课程 RAG 项目的 Vue 3 前端。侧栏提供“知识问答”“知识资料”和“学习说明”三个可用区域。页面默认进入“智能问答”，通过 FastAPI `POST /ask` 展示最终答案、原始引用来源、Embedding/LLM 模型和各阶段耗时；“语义检索”完整保留原有 `POST /search` Top-K 原文展示能力；“知识资料”通过 `GET /documents`、`POST /documents` 和 `DELETE /documents/{id}` 支持上传、查看和删除 TXT、Markdown、文本型 PDF 文档；“学习说明”介绍资料准备、有效提问、来源核对和当前格式边界。
...
智能问答和语义检索共用同一个问题输入框；进入知识资料或学习说明时输入框隐藏但保留状态。切换区域不会自动请求，也不会清除问答与检索最近的结果。智能问答处理 422 输入错误、502 生成失败、503 LLM 未配置、503 RAG 阈值配置无效和网络连接失败；问答不可用时仍可切换到语义检索。知识库管理处理 413 文件过大、415 类型不支持、422 空文件或解析失败、404 文档不存在、409 内置文档不可删除和网络错误；上传失败不影响已有文档，上传或删除成功后列表会自动刷新。上传和删除成功后无需重启应用即可在问答与检索中使用新内容。
...
如果只想离线查看全部界面、不启动 FastAPI，可以在启动 Vite 前设置：
...
学习说明：App.vue → StudyGuide（静态使用指引，不调用后端 API）
```
