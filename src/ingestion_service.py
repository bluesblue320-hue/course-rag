...
#: Stored filenames are always ``<32 lowercase hex digits>.<suffix>`` as
#: generated by :meth:`IngestionService.ingest`.  The strict format doubles
#: as a cross-platform path traversal guard: anything containing a path
#: separator, a dot segment, or an absolute path fails to match, so the
#: check never depends on the host OS's notion of a path separator.
_STORED_FILENAME_RE = re.compile(r"[0-9a-f]{32}\.(?:txt|md|pdf)")
...
class IngestionService:
    ...
    def _resolve_stored_path(self, stored_filename: str) -> Path:
        ...
        Stored filenames are always generated by :meth:`ingest` as a 32-hex
        UUID plus a supported suffix.  The name is validated against that
        exact format instead of relying on OS-specific path resolution, so
        path traversal attempts (``..``, absolute paths, or Windows
        backslash separators) are rejected consistently on every platform.
        """
